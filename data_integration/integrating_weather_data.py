...
def get_oneday_weather_observation_data(date, station, sql_db_fn, return_sql_df=False):
    ...
    output_dict['相對溼度(%)'] = [None if nanmean(sql_df['Humidity']) is None else round(nanmean(sql_df['Humidity']))]
    output_dict['風速(m/s)'] = [None if nanmean(sql_df['Wind_Speed']) is None else round(nanmean(sql_df['Wind_Speed']), ndigits=1)]
